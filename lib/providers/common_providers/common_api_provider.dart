...
import 'package:klpro_user/models/dashboard_user_model.dart';
...
class CommonApiProvider extends ChangeNotifier {
  ...
  getPaymentMethodList(context) async {
    ...
    String apiUrl;
    String? zoneIdList = pref.getString(session.zoneIds);
    if (zoneIdList != null && zoneIdList.isNotEmpty) {
      apiUrl =
          "${api.paymentMethod}?zone_ids=$zoneIdList";
    } else {
  ...
  Future getDashboardHome(BuildContext context) async {
    ...
    try {
      ...
      /* final response = await dioo.get(
        '${api.dashboardHome}?zone_ids=${pref.getString(session.zoneIds) ?? zoneIds}',
  ...
  Future getDashboardHome2(BuildContext context) async {
    ...
    try {
      ...
      String? lang = pref.getString("selectedLocale");
      /* final response = await dioo.get(
        '${api.dashboardHome1}?zone_ids=${pref.getString(session.zoneIds) ?? zoneIds}',
        options: Options(headers: { */
      final response = await dioo.get(
        api.dashboardHome1,
        options: Options(headers: {
