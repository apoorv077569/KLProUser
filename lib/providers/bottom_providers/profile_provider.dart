...
class ProfileProvider with ChangeNotifier {
  ...
  onTapOption(data, context, controller, sync) async {
    log(" message=-=-=-$data");
    final title = data is Map ? data['title'] : data.title;
    
    if (title == translations!.favouriteList) {
      if (isGuest) {
    ...
    } else if (title == "Provider in your area") {
      if (isGuest) {
        route.pushAndRemoveUntil(context);
      } else {
        route.pushNamed(context, routeName.userZoneServiceman);
      }
    } else if (title == translations!.manageLocations) {
      if (isGuest) {
        route.pushAndRemoveUntil(context);
      } else {
        route.pushNamed(context, routeName.myLocation);
      }
    } else if (title == translations!.myReviews) {
      if (isGuest) {
        route.pushAndRemoveUntil(context);
      } else {
    ...
    } else if (title == translations!.appDetails) {
      route.pushNamed(context, routeName.appDetails);
    } else if (title == translations!.rateUs) {
      if (isGuest) {
        route.pushAndRemoveUntil(context);
      } else {
        route.pushNamed(context, routeName.rateApp);
      }
    } else if (title == translations!.chatHistory) {
      if (isGuest) {
        route.pushAndRemoveUntil(context);
      } else {
    ...
    } else if (title == translations!.logOut) {
      logoutConfirmation(context);
    } else if (title == translations!.shareApp) {
      shareApp(context);
    } else if (title == translations!.helpSupport) {
      route.pushNamed(context, routeName.helpSupport);
    } else if (title == "Call") {
      launchCall(context, "+918287266266");
    } else if (title == "Chat With Staff") {
      route.pushNamed(context, routeName.chatWithStaffScreen);
    } else if (title == "Referral ID") {
      route.pushNamed(context, routeName.referralScreen);
    } else if (title == translations!.deleteAccount) {
      animateDesign(sync);
      showDialog(
          context: context,
          builder: (context1) {
  ...
  void shareApp(BuildContext context) {
    ...
    Share.share(
      'Download the Klpro User App for get better services at home.\n\nhttps://play.google.com/store/apps/details?id=com.softfix.ondemandservice',
