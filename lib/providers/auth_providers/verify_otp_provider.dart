...
class VerifyOtpProvider with ChangeNotifier {
  ...
  String? phone, dialCode, verificationCode, min, sec, email, uid, type;
  ...
  verifyOtp(context) async {
    ...
    try {
      String apiUrl = type == "register" ? api.verifyRegisterOtp : api.verifyOtp;
      await apiServices.postApi(apiUrl, jsonEncode(body)).then((value) {
        hideLoading(context);
        ...
        if (value.isSuccess!) {
          if (type == "register") {
            ...
            registerProvider.setEmailVerified(true, otp: otpController.text);
            route.pop(context);
