...
import 'package:klpro_user/firebase_options.dart';
import 'package:klpro_user/main.dart';
...
/// Flag set to true once splash/onReady has finished and the app is navigable.
bool isSplashReady = false;

/// Holds a notification message received during app cold-start (while splash is still loading).
...
class CustomNotificationController {
  ...
  Future<void> initNotification(context) async {
    ...
    //when app is [closed | killed | terminated] — opened via notification tap
    FirebaseMessaging.instance
        .getInitialMessage()
        .then((RemoteMessage? message) {
      if (message != null) {
        flutterLocalNotificationsPlugin.cancelAll();
        debugPrint("CHECK NOTI - initial message received");
        if (isSplashReady) {
          // App already ready — navigate immediately
          showFlutterNotification(message, true, context);
        } else {
          // Splash not done yet — store and process after splash finishes
          pendingInitialMessage = message;
          debugPrint("Notification stored — waiting for splash to finish");
        }
      }
    });
